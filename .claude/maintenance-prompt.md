...
4. A polite `reply_html` is fine: acknowledge the request, state it awaits admin review.
...
- `reply_html` must be valid Telegram HTML (`<b>`, `<i>`, `<code>`, `<a>` tags). Escape stray `<`, `>`, `&` in dynamic content (code identifiers, generic types, placeholder syntax like `<svc>`) — an unbalanced/unsupported tag makes Telegram reject the WHOLE message, and the bot does not currently retry on failure.
