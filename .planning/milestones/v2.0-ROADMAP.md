...
- [x] 12-02-PLAN.md — Land the one-shot backfill tool in the catalog service (Plan-deviation Rule 3: relocated from maintenance to catalog because Go internal-package visibility blocks cross-module imports): BackfillRunner with Shikimori half (kind/rating/material_source/studios) + AniList half (tags via ARM->AniList mapping), 5s/15s/60s 429 retry+backoff (Option A deviation), Makefile target make backfill-attributes. Idempotent re-runnable. (Wave 2) ✅ shipped 2026-05-06 — 7 commits, Shikimori half 2854/2854 succeeded, AniList half streaming asynchronously.
- [x] 12-03-PLAN.md — Implement S5 TF-IDF SignalModule with Kodik fallback, register in handler ensemble at weight 0.20 (full ensemble: 0.30·S1 + 0.20·S2 + 0.20·S3 + 0.10·S4 + 0.20·S5), add to UserOrchestrator module list, ship Phase-12 changelog entry. Top-3 ordering shift verified vs Phase-11 baseline. (Wave 3) ✅ shipped 2026-05-06 — 5 commits (4 task commits + 1 changelog), ui_audit_bot top-3 shifted post-redeploy (Re:Zero S4 → Chainsaw Man Recap at rank 3; rank-1 Final score 0.323 → 0.523).
