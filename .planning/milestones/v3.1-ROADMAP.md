...
- ✅ **v3.0 Universal Anime Scraper** — Phases 15-20 (shipped 2026-05-18; Phase 20 cutover over-rotated — regression repaired in v3.1 Phase 24)
- 🟡 **v3.1 Scraper Self-Healing** — Phases 21-26 (REOPENED 2026-05-19; 21-23 shipped 2026-05-13 with SCRAPER-HEAL-08 regression and audit gaps; 24-26 newly scoped to restore + resolve + expand)
...
### v3.1 Scraper Self-Healing (Phases 21-26) — REOPENED 2026-05-19

> v3.1 originally shipped 2026-05-13 with Phases 21-23 (16 SCRAPER-HEAL requirements). On 2026-05-18 the v3.0 Phase 20 cutover over-rotated and regressed SCRAPER-HEAL-08 (EnglishPlayer.vue deleted alongside HiAnime/Consumet). The original audit's BLK-INT-01 (hls3 host rotation) + W-INT-01..03 warnings were never closed. Phases 24-26 are the reopening: 24 restores the regressed surface, 25 resolves the audit gaps, 26 expands the provider pool.
...
- [ ] **Phase 24: EN Reconnect** — Restore the regressed `EnglishPlayer.vue` from git history (commit 8424e99, 1973 lines, three-phase loader intact), re-mount the EN tab in `Anime.vue`, re-widen the language/provider type whitelists, restore the minimum i18n key set. Hard gate before any frontend file touches: end-to-end provider verification against Frieren (MAL 52991) for every registered scraper provider — the operator's "test each provider" demand. See `.planning/milestones/v3.1-phases/24-en-reconnect/24-CONTEXT.md`.

### Phase 25: Audit Findings Resolution (REOPENED, planning)

- [ ] **Phase 25: Audit Findings Resolution** — Close the four 2026-05-13 audit gaps. (a) BLK-INT-01: trigger Plan 23-03 Task 4 manual smoke end-to-end so the maintenance-bot Pattern 7 dispatch self-heals the rotated hls3 allowlist; (b) W-INT-01: rewrite the parallel-probe race test in `gogoanime/client_gated_test.go`; (c) W-INT-02: sync `.claude/maintenance-prompt.md` Pattern 7 guidance to reference real scraper symbols; (d) W-INT-03: streaming-handler "domain not allowed" path returns HTTP 502 instead of silent 200. See `.planning/milestones/v3.1-phases/25-audit-findings-resolution/25-CONTEXT.md`.

### Phase 26: Provider Expansion (REOPENED, planning)

- [ ] **Phase 26: Provider Expansion** — Three deliverables: (a) lift `services/catalog/internal/parser/allanime/` into `services/scraper/internal/providers/allanime/` as a third in-rotation provider; (b) fresh 2026 EN-source survival sweep with an operator decision gate to pick 0-2 survivors to implement; (c) AnimeKai recovery — fill in the v3.0 Phase 19 escape-hatch stub with the in-house MegaUp token generator (carried-over `SCRAPER-KAI-01..04` + `SCRAPER-KAI-07`). EnglishPlayer in-player source dropdown lights up with 2-4 providers once expansion lands. See `.planning/milestones/v3.1-phases/26-provider-expansion/26-CONTEXT.md`.

### Next Milestone (TBD, was Phase 24/25 — renumbered)

After v3.1 ships, run `/gsd-new-milestone` to start the next cycle. Reserved future phases (now renumbered since 24/25 were absorbed into v3.1's reopening):
- Phase 27: VibePlayer Recovery via WARP egress (separate spec when there is appetite to revive VibePlayer as a working server)
- Phase 28: MinIO Hot Archival (separate v3.2 spec; rip + serve popular titles from MinIO)
