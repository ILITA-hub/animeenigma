...
- [x] **HSB-BE-01**: New endpoint `GET /api/home/spotlight` in `services/catalog`,
  ...
  become eligible.
- [x] **HSB-BE-02**: New package `services/catalog/internal/service/spotlight/`
  with `aggregator.go` and per-card resolvers under `cards/`. Each resolver
  implements `Resolve(ctx, userID *string) (Card, error)`.
- [x] **HSB-BE-03**: Each card resolver runs with a per-card context deadline
...
- [x] **HSB-BE-13**: `platform_stats` resolver. Computes up to 3 metrics:
...
- [x] **HSB-BE-20**: `personal_pick` resolver. Anon: calls `/api/anime/trending?limit=10`.
...
- [x] **HSB-BE-21**: `telegram_news` resolver. Reuses existing `parser/telegram`
  + cache `news:telegram` (TTL 30 min, untouched). Returns first 3 posts.
- [x] **HSB-BE-22**: `now_watching` resolver. SQL:
  ```sql
...
- [x] **HSB-BE-23**: New `services/catalog/internal/service/spotlight/client/player_client.go` —
  thin HTTP client to `http://player:8083` for cards 8 and 9.
- [x] **HSB-BE-24**: `not_time_yet` resolver (login only). Player API:
...
- [x] **HSB-BE-25**: `continue_watching_new` resolver (login only). Player API:
...
- [x] **HSB-BE-26**: Player exposes the two internal endpoints (or extends an
...
- [x] **HSB-BE-30**: For multi-item resolvers (`personal_pick`, `latest_news`,
...
- [x] **HSB-FE-01**: New component `frontend/web/src/components/home/spotlight/HeroSpotlightBlock.vue`.
...
- [x] **HSB-FE-02**: Fetches `GET /api/home/spotlight` once on mount via the
  existing API client. On error or empty response → block hides itself
  (`v-if="cards.length > 0"`).
- [x] **HSB-FE-03**: Auto-cycle interval **7s**. Pauses on `mouseenter` /
  `focusin` on the wrapper, resumes on `mouseleave` / `focusout`.
- [x] **HSB-FE-04**: Manual navigation — left/right chevrons + dot indicators
  (one per eligible card). Keyboard: `ArrowLeft` / `ArrowRight` seek when the
  block has focus; `Tab` cycles through dots.
- [x] **HSB-FE-05**: Initial slide chosen randomly (`randomInt(0, cards.length-1)`)
  on every page load.
- [x] **HSB-FE-06**: Respects `prefers-reduced-motion: reduce` — disables
  auto-cycle (manual nav still works), removes slide-transition animation.
- [x] **HSB-FE-07**: A11y — wrapper has `role="region"` + `aria-roledescription="carousel"`.
...
- [x] **HSB-FE-08**: Loading state — animated skeleton placeholder matching
  the block's final height (avoid layout shift).
- [x] **HSB-FE-09**: Feature flag env `VITE_HERO_SPOTLIGHT_ENABLED` (default
...
- [x] **HSB-MIG-01**: Phase 3 removes the `trendingRecs` markup + script state
...
- [x] **HSB-MIG-02**: After Phase 3 ships, drop the dual-flag setup. The
...
- [x] **HSB-NF-01**: Endpoint p95 latency **≤ 400ms** for cached responses
...
- [x] **HSB-NF-02**: `now_watching` SQL query uses an index on
...
- [x] **HSB-NF-03**: All new Redis keys use the `spotlight:` prefix for
  observability and bulk-flush.
- [x] **HSB-NF-04**: Privacy — `now_watching` exposes only `username` and
  ...
  the WHERE clause.
- [x] **HSB-NF-05**: Documentation — `CLAUDE.md` gets a new section under
