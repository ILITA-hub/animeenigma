...
- [ ] **NOTIF-NF-01**: Prometheus metrics on the notifications service `/metrics`:
...
- [ ] **NOTIF-NF-02**: Logging via `libs/logger` with structured fields. Detector runs log at INFO: `combos_scanned`, `affected_combos`, `notifications_upserted`, `duration_ms`, `parser_failures`. Per-combo errors log at WARN with combo context. No PII in logs (user_id is fine; usernames are not logged).
