# AnimeEnigma

[🇬🇧 English version](README.md)

Самохостируемая платформа для просмотра аниме с интеграцией MAL/Shikimori. Построена как монорепозиторий Go микросервисов с Vue 3 фронтендом.

**Целевая аудитория**: Самостоятельный хостинг для небольших групп (CDN не требуется).

## Возможности

- **Единый мультисорсовый стриминг** — aePlayer выбирает между собственными, Kodik, английскими scraper-, raw- и возрастными источниками; классический Kodik iframe остаётся резервным вариантом
- **Каталог по запросу** — Данные об аниме загружаются из Shikimori в реальном времени при поиске
- **Японские субтитры** — Оверлей с выделяемым текстом (ASS/SRT/VTT) через Jimaku.cc
- **Рейтинг OP/ED** — Оценка и просмотр опенингов и эндингов аниме
- **Мультиплеерная игра** — Угадывание опенингов/эндингов в реальном времени через WebSocket
- **Отслеживание прогресса** — История просмотров, вотчлисты, синхронизация позиции воспроизведения
- **Каталог и расписание** — Фильтрация по жанрам, статусу, расписание выхода серий
- **Мультиязычный UI** — Английский, русский, японский
- **Репорты ошибок** — Отправка багов прямо из плеера с диагностикой и уведомлениями в Telegram
- **API-ключи** — Программный доступ (например, импорт вотчлиста MAL)
- **Аутентификация** — JWT авторизация с ролевым доступом (пользователь/админ)
- **Авто-создание БД** — Базы данных и таблицы создаются автоматически при первом запуске

## Архитектура

```
┌─────────────┐                         ┌──────────────┐
│  Фронтенд   │◄───── REST/WS ────────►│   Gateway    │
│   (Vue 3)   │                         └──────┬───────┘
└──────┬──────┘                                │
       │              ┌────────┬───────┬───────┼────────┬──────────┐
       │              │        │       │       │        │          │
       │         ┌────▼──┐ ┌──▼───┐ ┌─▼────┐ ┌▼─────┐ ┌▼──────┐ ┌▼───────┐
       │         │ Auth  │ │Catalog│ │Stream│ │Player│ │ Rooms │ │ Themes │
       │         └───────┘ └──┬───┘ └──┬───┘ └──────┘ └───────┘ └────────┘
       │                      │        │
       │         ┌────────────┤        │
       │         │            │        │
       │         ▼            ▼        ▼
       │   ┌──────────┐ ┌─────────┐ ┌──────┐
       │   │Shikimori │ │ Парсеры │ │MinIO │
       │   │  API     │ │         │ └──────┘
       │   └──────────┘ └────┬────┘
       │                     │
       │           ┌────────┼────────┐
       │           ▼        ▼        ▼
       │        Kodik   Scraper   Library
       │        (RU)    (EN HLS)   (HLS)
       └────┘
```

### Видео плееры

Во фронтенде осталось две поверхности воспроизведения:

| Плеер | Язык | Технология | Особенности |
|-------|------|-----------|-------------|
| **aePlayer** | Несколько языков | HTML5 + hls.js | Единый выбор источника, субтитры, качество, трекинг |
| **Классический Kodik** | RU | Iframe | Резервная совместимость без прямого управления видео |

Видео получается тремя способами:

1. **Iframe (Kodik)** — Фронтенд встраивает плеер Kodik напрямую
2. **Проксированный поток** — Бэкенд разрешает и проксирует поддерживаемые внешние HLS/MP4-источники
3. **Собственное хранилище (MinIO)** — Загруженные админом видео

### Каталог по запросу

База данных аниме **НЕ заполняется заранее**. Вместо этого:

1. Пользователь ищет аниме во фронтенде
2. Сервис каталога запрашивает Shikimori GraphQL API
3. Результаты сопоставляются по **японскому названию** как первичному ключу
4. Метаданные аниме сохраняются в PostgreSQL для будущих запросов
5. Источники видео определяются через реестр провайдеров, адаптеры каталога и scraper-сервис

## Быстрый старт

### Требования

- Go 1.25+
- Bun 1.x+
- Docker & Docker Compose
- Make

### Разработка

1. **Запустить инфраструктуру:**
   ```bash
   make dev
   ```

2. **Запустить бэкенд сервисы:**
   ```bash
   # В отдельных терминалах или через docker compose
   cd services/auth && go run ./cmd/auth-api
   cd services/catalog && go run ./cmd/catalog-api
   # ... и т.д.
   ```

3. **Запустить фронтенд:**
   ```bash
   cd frontend/web
   bun install
   bun run dev
   ```

### С Docker Compose

```bash
# Запустить всё
docker compose -f docker/docker-compose.yml up -d

# Просмотр логов
docker compose -f docker/docker-compose.yml logs -f

# Остановить
docker compose -f docker/docker-compose.yml down
```

## Структура проекта

```
animeenigma/
├── services/           # Go микросервисы
│   ├── auth/           # Сервис аутентификации
│   ├── catalog/        # Каталог аниме с интеграцией Shikimori
│   ├── streaming/      # Сервис стриминга/прокси видео
│   ├── player/         # Прогресс просмотра, вотчлисты, репорты
│   ├── rooms/          # Игровые комнаты и WebSocket
│   ├── scheduler/      # Фоновые задачи
│   ├── themes/         # Рейтинги OP/ED
│   └── gateway/        # API шлюз
│
├── frontend/
│   └── web/            # Vue 3 SPA
│
├── libs/               # Общие Go библиотеки
│   ├── logger/         # Структурированное логирование
│   ├── errors/         # Обработка ошибок
│   ├── cache/          # Redis кэширование
│   ├── database/       # PostgreSQL с GORM (авто-создание)
│   ├── authz/          # JWT аутентификация
│   ├── httputil/       # HTTP утилиты
│   ├── pagination/     # Пагинация
│   ├── animeparser/    # Интерфейс парсеров видео источников
│   ├── idmapping/      # Маппинг ID аниме (MAL/Shikimori → AniList через ARM)
│   ├── videoutils/     # Работа с видео, HLS прокси, MinIO
│   ├── metrics/        # Prometheus метрики
│   └── tracing/        # OpenTelemetry
│
├── api/                # API контракты
│   ├── openapi/        # OpenAPI спецификации
│   ├── proto/          # Protobuf определения
│   ├── graphql/        # GraphQL схема
│   └── events/         # CloudEvents для асинхронных сообщений
│
├── docker/             # Docker Compose для локальной разработки
├── deploy/             # Kubernetes конфигурации
│   └── kustomize/
├── infra/              # Helm чарты
│   └── helm/
└── scripts/            # Скрипты сборки и утилиты
```

## Сервисы

| Сервис | Порт | Описание |
|--------|------|----------|
| Gateway | 8000 | API шлюз, rate limiting, маршрутизация |
| Auth | 8080 | Аутентификация, JWT, API-ключи |
| Catalog | 8081 | Каталог аниме, Shikimori, видео-парсеры |
| Streaming | 8082 | Стриминг/прокси видео, MinIO |
| Player | 8083 | Прогресс просмотра, вотчлисты, репорты |
| Rooms | 8084 | Игровые комнаты и WebSocket |
| Scheduler | 8085 | Фоновые задачи |
| Themes | 8086 | Рейтинги OP/ED |
| Frontend | 80 | Vue 3 SPA (nginx) |

## Конфигурация

Сервисы настраиваются через переменные окружения. См. `internal/config/config.go` каждого сервиса.

### Основные сервисы

| Переменная | Описание | По умолчанию |
|------------|----------|--------------|
| `JWT_SECRET` | Секрет для подписи JWT | - |
| `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, `DB_NAME` | PostgreSQL | localhost:5432 |
| `REDIS_HOST`, `REDIS_PORT` | Redis | localhost:6379 |
| `MINIO_ENDPOINT`, `MINIO_ACCESS_KEY`, `MINIO_SECRET_KEY` | MinIO хранилище | localhost:9000 |

### Видео и внешние сервисы

| Переменная | Описание | Обязательно |
|------------|----------|-------------|
| `KODIK_API_KEY` | API ключ Kodik для поиска RU видео | Для поддержки Kodik |
| `SHIKIMORI_CLIENT_ID` | OAuth client ID Shikimori | Опционально |
| `SHIKIMORI_CLIENT_SECRET` | OAuth секрет Shikimori | Опционально |
| `JIMAKU_API_KEY` | API ключ Jimaku.cc для JP субтитров | Для поддержки JP субтитров |
| `TELEGRAM_ADMIN_CHAT_ID` | Telegram chat ID для уведомлений | Для уведомлений о репортах |

Английские провайдеры обслуживает микросервис `scraper`; для браузерных провайдеров используется sidecar `stealth-scraper`.

### Пример `.env`

```env
# База данных (авто-создание если не существует)
DB_HOST=localhost
DB_PORT=5432
DB_USER=animeenigma
DB_PASSWORD=secret
DB_NAME=animeenigma

# Redis
REDIS_HOST=localhost
REDIS_PORT=6379

# MinIO (для загрузок админа)
MINIO_ENDPOINT=localhost:9000
MINIO_ACCESS_KEY=minioadmin
MINIO_SECRET_KEY=minioadmin
MINIO_BUCKET=animeenigma

# Аутентификация
JWT_SECRET=your-super-secret-key

# Провайдеры видео
KODIK_API_KEY=your-kodik-api-key

# Опционально
JIMAKU_API_KEY=your-jimaku-api-key
TELEGRAM_ADMIN_CHAT_ID=your-telegram-chat-id
```

## Разработка

```bash
# Запустить все Go тесты
make test

# Линтинг Go кода
make lint

# Собрать все сервисы
make build

# Сгенерировать API код
make generate

# Собрать Docker образы
make docker-build

# Пересобрать и перезапустить один сервис после изменений кода
make redeploy-catalog

# Следить за логами сервиса
make logs-catalog

# Проверить здоровье всех сервисов
make health
```

## Деплой

### Kubernetes с Kustomize

```bash
# Деплой в dev
kubectl apply -k deploy/kustomize/overlays/dev

# Деплой в prod
kubectl apply -k deploy/kustomize/overlays/prod
```

### Helm

```bash
cd infra/helm
helm install animeenigma ./gateway -f gateway/values.yaml
```

## Документация API

- OpenAPI спецификации: `api/openapi/`
- GraphQL схема: `api/graphql/schema.graphql`
- Proto определения: `api/proto/`

## Мультиплеерная игра

AnimeEnigma включает игру по угадыванию опенингов/эндингов:

1. Создайте комнату с настройками (количество раундов, время, режим)
2. Пригласите друзей по ссылке
3. Видео опенинга/эндинга проигрывается
4. Игроки вводят название аниме
5. Очки начисляются за скорость и правильность ответа
6. Глобальная и сессионная таблица лидеров

## Мониторинг

Каждый сервис предоставляет Prometheus метрики на `/metrics`:

- `http_requests_total` — Счётчик запросов с метками
- `http_request_duration_seconds` — Гистограмма задержек (p50/p95/p99)
- `http_response_size_bytes` — Гистограмма размера ответов

Дашборды Grafana и Prometheus доступны через контейнер admin-nginx при деплое.

## Лицензия

MIT
