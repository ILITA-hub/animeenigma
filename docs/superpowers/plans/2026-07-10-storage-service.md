...
- Modify: `services/streaming/cmd/streaming-api/main.go:56` + `services/streaming/internal/handler/stream.go:72-79` — build both storages, `proxyCfg.UpstreamSigner = multi.PresignURL`. `FirstPartyHosts` keeps ONLY the MinIO host — do NOT add the external S3 host (FirstPartyHosts exists to exempt Docker-private hosts from the SSRF/DNS-rebind dial guard; a public S3 host needs no exemption and listing it would strip rebind protection; presigning/entry-gate don't depend on it). [Amended 2026-07-10 after Task 6 review — original text said "gains both hosts", which was wrong.]
