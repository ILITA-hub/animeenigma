...
- **One provider, two legs.** `animejoy` is a single provider/family; Sibnet/AllVideo are **servers** resolved at play-time with Sibnet→AllVideo failover (not two separate providers). Per-leg health is still tracked for diagnostics.
...
### 3.2 Seed row: `services/catalog/internal/service/scraperprovider/seed.go`

```go
// defaultProviders (~line 19), after the kodik rows:
{Name: "animejoy", Status: domain.StatusDegraded /* soak first */,
 SupportsSub: true, SupportsDub: false, SupportsRaw: false,
 SubDelivery: "hard", QualityCeiling: "1080p", PreferenceWeight: 20,
 Reason: "Soaking — animejoy.ru RU-sub (Sibnet/AllVideo)",
 Description: "AnimeJoy — Russian-subtitled (Sibnet + AllVideo mirrors)"},
```
- `intrinsicGroups` (seed.go:187): add `"animejoy": "ru"`. **Do NOT** add to `scraperOperatedNames` (seed.go:207) — that pulls it into the EN scraper-failover chain and crash-loops boot.
- **Prod migration** (`scraperprovider/migrate.go` + ordering in `catalog-api/main.go:173-259`, after `SeedDefaults`): one-time guarded INSERT mirroring `AnimepaheBrowserRevival`/`SplitKodik` (server IS prod; seed is insert-if-absent only).

### 3.3 Capability family: `services/catalog/internal/service/capability/families_ru.go`

- `animejoyFamily(ctx, animeID)` (copy `kodikFamily`, :88): call `GetAnimejoyTeams`, build `[]domain.Variant{Category:"sub", Team:{ID,Name}, SubDelivery:"hard"}` (all `sub`), `providerRow(ctx,"animejoy")`, `applyFeedFields`. **MVP**: auto-pick best team (single variant); team chips = follow-up.
- `CatalogSource` iface (families_ru.go:11): add `GetAnimejoyTeams(ctx, animeID)([]domain.AnimejoyTeam, error)`; implement on `*CatalogService`; add stub to `fakeCatalog` (families_ru_test.go:22) or the package won't compile.
- `buildFamilies` (service.go:99-122, inside the `s.catalog != nil` guard): declare the `animejoy` slot, `wg.Add(1)` + goroutine, append in stable order (:118).
- DisplayName "AnimeJoy" in the labels map (`capability/rank.go:82`).
...
- Two-provider split (`animejoy-sibnet`/`animejoy-allvideo`) — revisit only if per-leg UI selection is wanted.
