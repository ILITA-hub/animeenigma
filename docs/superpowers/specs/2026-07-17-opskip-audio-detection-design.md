...
3. **Multi-OP titles:** long shows swap OPs mid-season. The fingerprint store allows MULTIPLE fingerprints per (anime, kind); matching tries all stored ones; when none match on ≥2 nearby episodes of the same unit family, those two episodes are paired to bootstrap an additional fingerprint.
...
- **catalog:** the existing `/api/skip-times/{malId}/{episode}` handler gains optional query params `anime=<uuid>&provider=&team=`. Resolution order: detected row for (anime, provider, team, episode) → **wins**; else AniSkip passthrough exactly as today. Detected responses reuse the AniSkip wire shape (`found/results[{interval, skipType, episodeLength}]`) plus `source: "detected"` so the FE needs no new parsing. Cached 10min (detected rows only grow).
- **FE:** `useSkipTimes` passes the active combo's animeId/provider/team; `useSkipIntro`/`skipSegments`/auto-skip settings work unchanged. Per-encode detected timings mean the skip lands exactly on the encode being played.
