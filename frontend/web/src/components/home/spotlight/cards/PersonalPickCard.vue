<template>
  <!--
    Workstream hero-spotlight — v1.1-polish Phase 04
    (HSB-V11-PP-01..04).

    Refactor: replace the 3-equal-poster grid with a two-zone layout:
      - Featured pick (60% desktop / full mobile): large poster +
        title + reason chip, backed by the featured anime's blurred poster.
    ...
    Title personalization: when `data.source === 'personal'` and the
    auth store has a `user.username`, render `titleWithName` with the
    username interpolated; falls back to `title` (logged-in, no name)
    or `titleAnon` (anonymous / source='trending').
  -->
  <article
    v-if="featured"
