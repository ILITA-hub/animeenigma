<template>
  <div class="consumet-player consumet-player-wrapper">
    ...
    <div v-else-if="episodes.length === 0 && !loadingEpisodes" class="text-center py-20 text-white/60">
      ...
      {{ $t('player.noEpisodes', { source: 'Consumet' }) }}
    </div>
      ...
      <div class="flex-1 min-w-0">
        <div ref="playerContainer" class="relative aspect-video bg-black rounded-xl overflow-hidden">
          ...
          <!-- Phase 18 (UX-34) — Skip-Intro / Skip-Outro CTAs.
               Mirrors HiAnimePlayer.vue. bottom-20 keeps clear of native
               video controls; z-10 sits above controls but below modal
               dialogs. v-else-if guarantees OP and ED never stack. -->
          <button
...
// stay null and showSkipIntro/showSkipOutro evaluate false — CTA buttons
// render nothing. Pure additive feature, mirrors HiAnimePlayer.vue.
...
const markCurrentEpisodeWatched = async () => {
  ...
  try {
    ...
    // = no emit (not all marks originate from a recs row click).
