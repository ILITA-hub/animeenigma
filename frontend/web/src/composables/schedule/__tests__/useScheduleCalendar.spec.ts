...
import type { ScheduleAnime, ScheduleConfirmedOccurrence } from '../types'
...
describe('useScheduleCalendar', () => {
  ...
  it('keeps entries visible after navigating to the previous week', () => {
    const history = ref<ScheduleConfirmedOccurrence[]>([
      { anime_id: '1', episode: 9, aired_at: '2026-06-01T17:00:00Z', source: 'anilist', anime: data[0] },
      { anime_id: '2', episode: 22, aired_at: '2026-06-03T20:00:00Z', source: 'anilist', anime: data[1] },
    ])
    const cal = useScheduleCalendar({
      animes: ref(data),
      confirmedOccurrences: history,
      now: ref(new Date(2026, 5, 8)),
      statusOf: () => null,
      loggedIn: ref(false),
    })
  ...
  it('includes historical anime that are no longer in the upcoming feed', () => {
    const released = { ...data[0], id: 'released', status: 'released', next_episode_at: null }
    const cal = useScheduleCalendar({
      animes: ref(data),
      confirmedOccurrences: ref([
        { anime_id: released.id, episode: 12, aired_at: '2026-06-01T17:00:00Z', source: 'anilist', anime: released },
      ]),
      now: ref(new Date(2026, 5, 1)),
      statusOf: () => null,
      loggedIn: ref(false),
    })

    expect(cal.allAnimes.value.map(anime => anime.id)).toContain('released')
    expect(cal.weekColumns.value.flatMap(cell => cell.occurrences).map(o => o.episode)).toContain(12)
  })
})
