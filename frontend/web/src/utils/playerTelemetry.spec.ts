import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest'
import {
  recordPlayerEvent,
  flushPlayerTelemetry,
  __resetPlayerTelemetryForTest,
} from './playerTelemetry'

// Transport is fetch-keepalive only (shipAnalyticsPayload, AUTO-629):
// sendBeacon must never be touched — $ping-class blockers eat beacons after
// sendBeacon() returns true, which silently zeroed telemetry.
describe('playerTelemetry', () => {
  beforeEach(() => {
    __resetPlayerTelemetryForTest()
    Object.defineProperty(navigator, 'sendBeacon', {
      configurable: true,
      writable: true,
      value: vi.fn(() => true),
    })
    vi.stubGlobal('fetch', vi.fn(() => Promise.resolve()))
  })

  afterEach(() => {
    __resetPlayerTelemetryForTest()
    vi.unstubAllGlobals()
  })

  it('buffers two events and flushes them as a single fetch batch (no sendBeacon)', () => {
    recordPlayerEvent({ kind: 'resolve', provider: 'gogoanime', anime_id: 'abc', outcome: 'ok', latency_ms: 200 })
    recordPlayerEvent({ kind: 'stall', provider: 'gogoanime', anime_id: 'abc', stall_ms: 3000 })

    flushPlayerTelemetry('test')

    expect(navigator.sendBeacon as ReturnType<typeof vi.fn>).not.toHaveBeenCalled()
    const fetchMock = fetch as unknown as ReturnType<typeof vi.fn>
    expect(fetchMock).toHaveBeenCalledTimes(1)
    const [url, opts] = fetchMock.mock.calls[0]
    expect(String(url)).toContain('/analytics/player-events')
    expect((opts as RequestInit).keepalive).toBe(true)
    const body = JSON.parse((opts as RequestInit).body as string)
    expect(body.events).toHaveLength(2)
    expect(body.events[0]).toMatchObject({ kind: 'resolve', provider: 'gogoanime', anime_id: 'abc' })
    expect(body.events[1]).toMatchObject({ kind: 'stall', provider: 'gogoanime', stall_ms: 3000 })
  })

  it('ignores events with empty provider', () => {
    recordPlayerEvent({ kind: 'resolve', provider: '', anime_id: 'abc' })
    recordPlayerEvent({ kind: 'resolve', provider: '  ', anime_id: 'abc' })
    flushPlayerTelemetry('test')
    expect(fetch).not.toHaveBeenCalled()
  })

  it('accepts playback_start_rejected events with the DOMException name in error_kind', () => {
    const fetchMock = fetch as unknown as ReturnType<typeof vi.fn>

    recordPlayerEvent({
      kind: 'playback_start_rejected',
      provider: 'kodik',
      anime_id: 'abc',
      episode: 6,
      error_kind: 'NotAllowedError',
    })
    flushPlayerTelemetry('test')

    expect(fetchMock).toHaveBeenCalledTimes(1)
    const body = JSON.parse((fetchMock.mock.calls[0][1] as RequestInit).body as string)
    expect(body.events).toHaveLength(1)
    expect(body.events[0]).toMatchObject({
      kind: 'playback_start_rejected',
      provider: 'kodik',
      error_kind: 'NotAllowedError',
    })
  })

  it('ignores events with invalid kind', () => {
    // TypeScript would prevent this, but at runtime bad data can arrive
    recordPlayerEvent({ kind: 'unknown' as 'resolve', provider: 'gogoanime', anime_id: 'abc' })
    flushPlayerTelemetry('test')
    expect(fetch).not.toHaveBeenCalled()
  })

  it('flush is a no-op when the buffer is empty', () => {
    flushPlayerTelemetry('test')
    expect(fetch).not.toHaveBeenCalled()
  })

  it('rate-caps events above RATE_PER_MIN and drops the excess', () => {
    // RATE_PER_MIN = 60 in playerTelemetry
    // Push 80 distinct events — first 60 accepted, rest dropped.
    // When 60 fills a batch of MAX_BATCH=20, 3 size-flushes fire → fetch called 3 times.
    for (let i = 0; i < 80; i++) {
      recordPlayerEvent({ kind: 'resolve', provider: `p${i}`, anime_id: `a${i}`, outcome: 'ok' })
    }
    // Drain the cap marker.
    flushPlayerTelemetry('manual')

    const fetchMock = fetch as unknown as ReturnType<typeof vi.fn>
    // 3 size-flushes for the 60 accepted events + 1 manual flush for the cap
    // marker; events after the cap are silently dropped → bounded by 4.
    expect(fetchMock.mock.calls.length).toBeGreaterThanOrEqual(3)
    expect(fetchMock.mock.calls.length).toBeLessThanOrEqual(4)
    const totalShipped = fetchMock.mock.calls
      .map((c) => (JSON.parse((c[1] as RequestInit).body as string).events as unknown[]).length)
      .reduce((a, b) => a + b, 0)
    expect(totalShipped).toBeLessThanOrEqual(61) // 60 real + 1 cap marker
  })

  it('accepts playback_failed and serializes its detail bundle', () => {
    recordPlayerEvent({
      kind: 'playback_failed',
      provider: 'ae',
      anime_id: 'abc',
      episode: 3,
      error_kind: 'stream_error',
      detail: { reason: 'ae_failed', all_exhausted: false, engine: { bw_bps: 1234 } },
    })
    flushPlayerTelemetry('manual')

    const fetchMock = fetch as unknown as ReturnType<typeof vi.fn>
    expect(fetchMock).toHaveBeenCalledTimes(1)
    const body = JSON.parse((fetchMock.mock.calls[0][1] as RequestInit).body as string)
    expect(body.events[0].kind).toBe('playback_failed')
    expect(body.events[0].detail.reason).toBe('ae_failed')
    expect(body.events[0].detail.engine.bw_bps).toBe(1234)
  })
})

describe('recordPlayerEvent — protocol_usage kind', () => {
  it('accepts and buffers a protocol_usage event', () => {
    __resetPlayerTelemetryForTest()
    const fetchSpy = vi.fn().mockResolvedValue(undefined)
    vi.stubGlobal('fetch', fetchSpy)

    recordPlayerEvent({
      kind: 'protocol_usage',
      provider: 'gogoanime',
      anime_id: 'a1',
      episode: 1,
      audio: 'sub',
      lang: 'en',
      detail: { protocol: 'h2', tier: 'h2', segments: 214 },
    })
    flushPlayerTelemetry('test')

    expect(fetchSpy).toHaveBeenCalledTimes(1)
    const body = (fetchSpy.mock.calls[0][1] as RequestInit).body as string
    expect(body).toContain('"kind":"protocol_usage"')
    expect(body).toContain('"protocol":"h2"')
    vi.unstubAllGlobals()
  })
})
