...
func (h *PlayerHealthChecker) checkConsumet() error {
	...
	// Step 1: Search
	body, err := h.httpGet(h.consumetURL + "/anime/animepahe/naruto")
	if err != nil {
		return fmt.Errorf("search failed: %w", err)
	}
	...
	body, err = h.httpGet(fmt.Sprintf("%s/anime/animepahe/info/%s", h.consumetURL, url.PathEscape(animeID)))
	if err != nil {
	...
	// Step 3: Get stream
	body, err = h.httpGet(fmt.Sprintf("%s/anime/animepahe/watch?episodeId=%s", h.consumetURL, url.QueryEscape(episodeID)))
	if err != nil {
