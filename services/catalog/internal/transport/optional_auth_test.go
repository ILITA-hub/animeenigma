...
//
// Verbatim port of services/player/internal/transport/optional_auth_test.go.
// The behavioral contract is FROZEN across both services:
//   - missing Authorization header → next handler called WITHOUT claims
//   - valid Bearer JWT → next handler called WITH claims attached
//   - malformed/expired Bearer JWT → next handler called WITHOUT claims
//     (i.e. the middleware NEVER rejects with 401 — that's the inversion vs.
//      AuthMiddleware)
//
// Plan 04 wires this middleware onto the catalog `/home/spotlight` route so
// login-only resolvers (PersonalPick login path, NotTimeYet, ContinueWatchingNew)
// receive userID-bearing claims when callers send a JWT.
...
func TestOptionalAuth_MalformedJWT_PassesThroughWithoutClaims(t *testing.T) {
	...
	assert.True(t, *called, "malformed JWT must NOT cause rejection — handler downstream falls back to anon")
