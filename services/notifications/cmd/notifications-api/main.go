...
//  7. Wire repos → service → handlers (public + internal).
//  8. transport.NewRouter
...
func main() {
	...
	// snapshotRepo is constructed for forward-compat — Phase 2's detector
	// pulls it from the boot graph rather than wiring fresh.
	_ = repo.NewSnapshotRepository(db.DB)
	...
	// Router.
	router := transport.NewRouter(notifHandler, internalHandler, cfg.JWT, log, metricsCollector)
