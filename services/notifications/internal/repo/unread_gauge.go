...
// historical-but-dismissed rows. Invalidated (tombstoned) rows are excluded so
// the gauge stays consistent with the per-user badge counts shown in the UI.
...
func (r *UnreadGaugeRepository) ActiveUnreadCount(ctx context.Context) (int64, error) {
	...
	err := r.db.WithContext(ctx).
		...
		Where("dismissed_at IS NULL AND read_at IS NULL AND invalidated_at IS NULL").
		Count(&n).Error
	if err != nil {
