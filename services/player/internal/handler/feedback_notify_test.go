...
func postInternalStatus(t *testing.T, h *AdminReportsHandler, id, status string) *httptest.ResponseRecorder {
	t.Helper()
	req := httptest.NewRequest(http.MethodPatch, "/internal/feedback/"+id+"/status",
...
func TestSetStatusInternal_ResolvedDoesNotNotify(t *testing.T) {
	// "resolved" is allowed on the internal route (the maintenance bot sets
	// it for info_only items) but has NO stage notification — the ai_done
	// thank-you, if any, stays as-is.
	fake := &fakeNotifications{}
	srv := fake.server(t)
	defer srv.Close()
	...
	if w.Code != http.StatusOK {
		t.Fatalf("resolved via internal route: code = %d, want 200", w.Code)
	}
	time.Sleep(150 * time.Millisecond)
	fake.mu.Lock()
	defer fake.mu.Unlock()
	if len(fake.upserts) != 0 || len(fake.invals) != 0 {
		t.Errorf("resolved must not dispatch, got upserts=%v invals=%v", fake.upserts, fake.invals)
	}
}
