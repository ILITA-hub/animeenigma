...
func TestUserOrchestrator_RunOnceDeletesCacheForSucceededUsersOnly(t *testing.T) {
	...
	assert.True(t, cache.deletesContains("recs:user:user-B:topN"),
	...
	assert.False(t, cache.deletesContains("recs:user:user-A:topN"),
...
func TestUserOrchestrator_TriggerForUser_AcquiresAndSpawns(t *testing.T) {
	...
	deadline = time.Now().Add(1 * time.Second)
	for time.Now().Before(deadline) {
		if cache.deletesContains("recs:user:user-A:topN") {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	assert.True(t, cache.deletesContains("recs:user:user-A:topN"),
