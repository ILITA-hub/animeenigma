...
func main() {
	...
	tracer, err = tracing.InitFromEnv(context.Background(), "rooms")
	if err != nil {
	...
	wsHandler := handler.NewWebSocketHandler(wsService, log, cfg.AllowedOrigins, cfg.JWT)
	...
	srv := &http.Server{
		...
		Handler:      tracing.HTTPMiddleware("rooms")(router),
