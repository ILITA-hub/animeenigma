...
// Lifted VERBATIM from services/catalog/internal/parser/allanime/decrypt.go
// (CONTEXT.md D1). Algorithm is upstream-defined — the rolling AES-CTR pattern
// is consumer-agnostic and identical for raw-jp and EN-sub use.
...
// is reserved for the auth tag and 0 for the J0 init). We discard the
// trailing tag and decrypt as plain CTR — AllAnime's web client doesn't
// verify it.
func decryptTobeparsed(blob string) ([]byte, error) {
	...
	// CTR initial counter = IV || 0x00000002 (32-bit big-endian, start at 2).
	ctr := make([]byte, 16)
	copy(ctr, iv)
