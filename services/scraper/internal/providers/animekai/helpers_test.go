...
// fakeCache is an in-memory cache.Cache implementation for tests. Lifted
// verbatim from gogoanime/helpers_test.go so the test rig is identical
// across providers (libs/cache exposes no in-memory implementation; every
// provider supplies its own helper). Order-sensitive logs (getLog/setLog)
// capture every Get/Set call so assertions can verify the cache key shapes.
