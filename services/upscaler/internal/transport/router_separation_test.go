...
func buildUpscalerRouter(t *testing.T) http.Handler {
	...
	return NewRouter(log, sharedUpscalerCollector(), hub, nil, segHandler, nil, nil, nil, nil, false)
}
...
func TestUpscalerRouter_AdminJobsResolveWithGatewayHeader(t *testing.T) {
	...
	router := NewRouter(log, sharedUpscalerCollector(), hub, nil, segHandler, nil, nil, nil, nil, false)
...
func TestRouter_AdminJobsWithRealHandler(t *testing.T) {
	...
	router := NewRouter(log, sharedUpscalerCollector(), hub, nil, segH, adminH, nil, nil, nil, false)
